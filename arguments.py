...
def add_runtime_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--type", type=str, default="")
...
def add_hp_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument('--decay-iters', type=float, default=0.01,
                       help='percentage of data to decay on (.01 = 1% of all '
                       'training iters). Default 0.01')
    group.add_argument('--lr-decay-iters', type=int, default=None,
                       help='number of iterations to decay LR over,'
                       ' If None defaults to `--train-iters`*`--epochs`')
    group.add_argument('--lr-decay-style', type=str, default='noam',
                       choices=['constant', 'linear', 'cosine', 'exponential', 'noam', 'wsd'],
...
def add_distillm_args(parser: argparse.ArgumentParser):
    ...
    # on-policy
    group.add_argument("--gkd", action="store_true")
    group.add_argument("--gkd-alpha", type=float, default=0.5)
    return parser

def add_AdaKD_args(parser: argparse.ArgumentParser):
    group = parser.add_argument_group('AdaKD', 'AdaKD configurations')

    # ctkd
    group.add_argument("--dynamic-temperature", action="store_true")
    group.add_argument("--start-value", type=float, default=1.0)
    group.add_argument("--end-value", type=float, default=0.0)
    # dtkd
    group.add_argument("--dtkd", action="store_true")
    # logits standardization/NormKD
    group.add_argument("--NormKD", action="store_true")
    
    # model ema
    group.add_argument("--model-ema", action="store_true")
    group.add_argument("--ema-decay", type=float, default=0.995)
    
    # AdaKD
    group.add_argument("--rule", type=str, default="reversekl")
    group.add_argument("--loss-fn", type=str, default="rkd")
    # LATF
    group.add_argument("--ema-decay-loss", type=float, default=0.99)
    group.add_argument("--loss-ratio-eps", type=float, default=0.1)
    group.add_argument("--sample-ratio-eps", type=float, default=0.1)
    # IDTS
    group.add_argument("--adaptive-temperature", action="store_true")
    group.add_argument("--temperature-scale", type=float, default=0.6)
    group.add_argument("--temperature-soften", action="store_true")
    return parser
...
def add_wandb_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--wandb-project", type=str, default="AdaKD")
...
def get_args():
    ...
    if args.type == "eval_main":
        ...
        args.save = save_path
    elif "kd" in args.type or "jsd" in args.type:
        save_path = os.path.join(
            args.save,
            (f"{args.ckpt_name}" + f"-{args.peft_name}" if args.peft_name is not None else "" + \
