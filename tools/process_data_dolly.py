...
class Encoder(object): 
    ...
    def encode(self, line):
        ...
        if "input" not in line or len(line["input"]) == 0:
            ...
            elif self.args.model_type=="rwkv":
                template = (
        ...
        if len(prompt_tokens) > self.args.max_prompt_length:
            print(f"Prompt length {len(prompt_tokens)} exceeds max length {self.args.max_prompt_length}")
    ...
    for split in all_data:
        ...
        for lid, (line, prompt_str, prompt, response, bytes_processed) in enumerate(encoded_docs):
            ...
            if args.only_prompt:
                ...
                # binary_builder.add_item(torch.IntTensor(prompt + response))
